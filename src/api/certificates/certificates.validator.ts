...
export class CertificatesValidator extends BaseValidator {
    public validateCreateRequest = async (request: express.Request): Promise<CertificatesCreateModel> => {
        try {
    ...
    public validateUpdateRequest = async (request: express.Request): Promise<CertificatesUpdateModel> => {
        try {
            const certificates = joi.object({
                ...
                CertificateUrl: joi.string().max(1000).min(0).optional(),
    ...
    public validateSearchRequest = async (request: express.Request): Promise<CertificatesSearchFilters> => {
        try {
            const certificates = joi.object({
                userId: joi.string().uuid().optional(),
                certificateNumber: joi.string().max(64).min(0).optional(),
