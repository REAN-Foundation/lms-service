...
export class CourseController {
    ...
    create = async (request: express.Request, response: express.Response) => {
        try {
            var model: CourseCreateModel = await this._validator.validateCreateRequest(request);
            const record = await this._service.create(model);
            if (record === null) {
                ErrorHandler.throwInternalServerError('Unable to add course!');
            }
    ...
    update = async (request: express.Request, response: express.Response) => {
        try {
            ...
            var model: CourseUpdateModel = await this._validator.validateUpdateRequest(request);
