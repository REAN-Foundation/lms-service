...
export class UserLearningValidator extends BaseValidator {
    ...
    public validateCreateRequest = async (request: express.Request)
        : Promise<UserLearningCreateModel> => {
        try {
            const user_learnings = joi.object({
                                UserId: joi.string().uuid().required(),
                ActionId: joi.string().uuid().required(),
                ProgressStatus: joi.string().valid(...Object.values(ProgressStatus)).required(),
    ...
    public validateUpdateRequest = async (request: express.Request)
        : Promise<UserLearningUpdateModel> => {
        try {
            const user_learnings = joi.object({
                                UserId: joi.string().uuid().optional(),
                ActionId: joi.string().uuid().optional(),
                ProgressStatus: joi.string().valid(...Object.values(ProgressStatus)).optional(),
    ...
    public validateSearchRequest = async (request: express.Request)
        : Promise<UserLearningSearchFilters> => {
        try {
            const user_learnings = joi.object({
                                userId: joi.string().uuid().optional(),
                actionId: joi.string().uuid().optional(),
