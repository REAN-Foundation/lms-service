...
_courseRepository: Repository<Course> = Source.getRepository(Course);
    ...
    public create = async (createModel: LearningPathCreateModel)
...
Enabled : createModel.Enabled,
    ...
    public search = async (filters: LearningPathSearchFilters)
        : Promise<LearningPathSearchResults> => {
        try {
    ...
    public update = async (id: uuid, model: LearningPathUpdateModel)
        : Promise<LearningPathResponseDto> => {
        try {
